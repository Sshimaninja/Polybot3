...
export async function lowSlippage(trade: Trade, slippageTolerance: BN,) {
...
//   const slippageAmount = BN(trade.recipient.tokenOutPrice).multipliedBy(slippageTolerance); 
//   const targetPrice = BN(trade.recipient.tokenOutPrice).minus(slippageAmount);
//   const amountRequired = BN(trade.recipient.reserve1).dividedBy(targetPrice).minus(trade.recipient.reserve0); 
//   const flashAmount = amountRequired.lt(BN(trade.loanPool.reserve1)) ? amountRequired : BN(trade.loanPool.reserve1);
//   return flashAmount;
// }
  let flashB1 = BN(0);
  let flashB0 = BN(0);
...
aReserve0 = 100 ETH
aReserve1 = 200, 000 USD
aPrice = 2,000- (2000 * 0.03) = 1,940 USD
amountOut = (aReserve0 / aPrice) - aReserveX
...
amountIn = ( Reserve0 / Price0 ) - Reserve1

amountIn = (bReseve1FN.div(bPrice1FN).sub(bReserve0))
