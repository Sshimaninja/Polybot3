...
export class TokenProfits {
    ...
    async getDirect(): Promise<{
        ...
        const directRepayLoanPoolInTokenOut = await this.q.minIn(repay);
        const directRepayLoanPoolInTokenOutWithFee = await addFee(
            directRepayLoanPoolInTokenOut.amountIn,
        );
        const profit =
