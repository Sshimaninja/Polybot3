...
export class InRangeLiquidity {
	...
	async getTokenAmounts(): Promise<{ amount0wei: number, amount1wei: number, inRangeReserves0: string, inRangeReserves1: string }> {
		const liquidity = await this.pool.liquidity();
		if (liquidity.isZero()) {
			console.log("Liquidity is zero for ", this.token0.id, this.token1.id, " on ", this.poolInfo.exchange, ". Skipping...")
			return { amount0wei: 0, amount1wei: 0, inRangeReserves0: '0', inRangeReserves1: '0' };
		} else {
			...
			let amount0Human = (amount0wei / (10 ** this.token0.decimals)).toFixed(this.token0.decimals);
			let amount1Human = (amount1wei / (10 ** this.token1.decimals)).toFixed(this.token1.decimals);
