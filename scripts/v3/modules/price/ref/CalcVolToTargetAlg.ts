    ...
    async calcVolToTarget(): Promise<bigint> {
        ...
        try {
            if (this.sPriceTarget > sPriceCurrent) {
                ...
                console.log(
                    "token0 tradeSize: ",
                    deltaTokens / 10 ** this.token0.decimals,
                    "from ",
                    uniswapV3Exchange[this.exchange].protocol,
                );
            ...
            if (sPriceCurrent < this.sPriceTarget) {
                ...
                while (this.sPriceTarget < sPriceCurrent) {
                    if (this.sPriceTarget < sPriceLower) {
                        ...
                        deltaTokens += this.y_in_range(
                            liquidity,
                            sPriceCurrent,
                            sPriceLower,
                        );
                        //  query the blockchain for liquidity in the previous tick range
                        tickLower = currentTickRange = await this.pool.ticks(
                            tickLower,
                        );
                        ...
                        currentTickRange = await this.pool.ticks(
                            BigInt(tickLower),
                        );
                    } else {
                        // in the current price range
                        let y = this.y_in_range(
                            liquidity,
                            sPriceCurrent,
                            this.sPriceTarget,
                        );
                    ...
                    console.log(
                        "token1 tradeSize: ",
                        deltaTokens / 10 ** this.token1.decimals,
                        "from ",
                        uniswapV3Exchange[this.exchange].protocol,
                    );
