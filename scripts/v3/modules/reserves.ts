...
import { ReservesData, Pool } from "../../../constants/interfaces";
/**
 * @description
...
export class ReservesV3 {
    ...
    constructor(match: Pool) {
        this.getReserves(match)
    }

    async getPoolIDs(pair: Pool): Promise<string[]> {
        const poolIDs: string[] = [];
        for (const key in pair) {
            if (key.startsWith("pool")) {
                const poolID = pair[key as keyof Pool];
    ...
    async getReserves(match: Pool): Promise<ReservesData[]> {
...
export class InRangeLiquidity {
    ...
    async getPoolIDs(pool: Pool): Promise<string[]> {
        const poolIDs: string[] = [];
        for (const key in pool) {
            if (key.startsWith("pool")) {
                const poolID = pool[key as keyof Pool];
                if (typeof poolID === "string") {
                    poolIDs.push(poolID);
                }
            }
    ...
    async getLiquidity(match: Pool): Promise<BigNumber[]> {
        const poolIDs = await this.getPoolIDs(match);
