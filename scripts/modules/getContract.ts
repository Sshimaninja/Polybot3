...
import { algebraFactory, algebraQuoter, uniswapV3Factory, uniswapV2Factory, uniswapQuoter, uniswapV2Router } from "../../constants/addresses";
...
export function getFactory(exchange: string): Contract {
	if (uniswapV2Factory[exchange]) {
		return new Contract(uniswapV2Factory[exchange], IUniswapV2Factory, signer)
	}
	if (uniswapV3Factory[exchange]) {
		return new Contract(uniswapV3Factory[exchange], IUniswapV3Factory, signer)
	}
	if (algebraFactory[exchange]) {
...
export function getRouter(exchange: string): Contract {
	if (uniswapV2Router[exchange]) {
		return new Contract(uniswapV2Router[exchange], IUniswapV2Router02, signer)
	}
	else {
		throw new Error("Exchange not supported");
	}
