...
export async function getProfitInMatic(trade: BoolTrade): Promise<MaticProfit> {
	...
	if (trade.tokenOut.id == matic) {
		...
		let gasPool = new Contract(await trade.target.pool.getAddress(), IPair, wallet);
	...
	if (trade.tokenIn.id == matic) {
		...
		let inMatic = await getAmountsOut(trade.profit, trade.target.reserveOut, trade.target.reserveIn)
	...
	if (g == undefined) {
		let maticProfit = { profitInMatic: 0n, gasPool: new Contract(await trade.target.pool.getAddress(), IPair, wallet) };
		return maticProfit
	}
	...
	if (g != undefined) {
		...
		if (token1 == matic) {
			logger.info("[getProfitInMatic]: Case 1: trade.tokenOut.id is paired with WMATIC")
		...
		if (token0 == matic) {
			logger.info("[getProfitInMatic]: Case 2: trade.tokenOut.id is paired with a WMATIC, but is in trade.tokenInID position")
			const reserves = await gasPool.getReserves();
		...
		if ((token0 && token1 != matic) && (token0 == trade.tokenOut.id)) {
			logger.info("[getProfitInMatic]: Case 3: trade.tokenOut.id is paired with a gasToken in the trade.tokenIn position")
			const reserves = await gasPool.getReserves();
			const profitInGasToken = await getAmountsOut(trade.profit, reserves[0], reserves[1]);//returns profit in gasToken/WMATIC
			const gasMaticPool = await (trade.loanPool.factory.getPair(token1, matic) ?? trade.target.factory.getPair(token1, matic) ?? undefined);
			console.log(gasMaticPool.token1, matic)
			const gasSMaticPoolContract = new ethers.Contract(gasMaticPool, IPair, provider);
			const profitInMatic = await gasSMaticPoolContract.token1() == matic ? await getAmountsOut(trade.profit, reserves[0], reserves[1]) : await getAmountsOut(profitInGasToken, reserves[1], reserves[0]);
			// const profitInMatic = pu(profitInMaticBN.toFixed(18), 18)
			let maticProfit = { profitInMatic, gasPool };
			return maticProfit
		}
		...
		if ((token0 && token1 != matic) && (token1 == trade.tokenOut.id)) {
			logger.info("[getProfitInMatic]: Case 4: trade.tokenOut.id is paired with a gasToken in the trade.tokenOut position")
			const reserves = await gasPool.getReserves();
			const profitInGasToken = await getAmountsOut(trade.profit, reserves[1], reserves[0]);//returns profit in gasToken/MWATIC
			const gasMaticPool = await trade.loanPool.factory.getPair(gasPool.token0(), matic) ?? trade.target.factory.getPair(gasPool.token0(), matic) ?? undefined;
			console.log(gasMaticPool.token0, matic)
			const gasSMaticPoolContract = new ethers.Contract(gasMaticPool, IPair, provider);
			const profitInMatic = await gasSMaticPoolContract.token1() == matic ? await getAmountsOut(trade.profit, reserves[1], reserves[0]) : await getAmountsOut(profitInGasToken, reserves[0], reserves[1]);
			// const profitInMatic = pu(profitInMaticBN.toFixed(18), 18)
			let maticProfit = { profitInMatic, gasPool };
			return maticProfit
		}
	...
	return { profitInMatic: 0n, gasPool: new Contract(await trade.target.pool.getAddress(), IPair, wallet) };
}
