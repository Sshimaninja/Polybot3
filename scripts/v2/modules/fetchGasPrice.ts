...
export async function fetchGasPrice(trade: BoolTrade): Promise<GAS> {
	...
	if (trade.direction != undefined) {
		...
		try {
			...
			gasEstimate = await trade.flash.getFunction("flashSwap").estimateGas(
			...
			return { gasEstimate, tested: false, gasPrice: BigInt(150 + 60 * 300000), maxFee, maxPriorityFee };
		}
		...
		const gasLogs = {
			...
			gasEstimateTimesMaxFeePlusMaxPriorityFeeTimes10: fu((gasEstimate * (maxFee + (maxPriorityFee)) + (maxFee)) * (BigInt(10)), 18)
		}
		console.log(gasLogs);
		...
		return { gasEstimate: BigInt(300000), tested: false, gasPrice: BigInt(150 + 60 * 300000), maxFee: maxFee, maxPriorityFee: maxPriorityFee };
	}
}
