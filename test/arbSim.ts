...
async function arbSim() {
	...
	async function getWMATIC(): Promise<BigInt | undefined> {
		if (wmaticBal.eq(0)) {
			...
			const maticApprove = await matic.approve(wmatic.address, ninetyFivePercentMatic, { gasLimit: gasLimit });
			...
			const ninetyFivePercentMatic = await wmatic.balanceOf(signer.address);
			return ninetyFivePercentMatic;
		} else {
