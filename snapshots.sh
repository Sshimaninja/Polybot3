  ...
  declare -A processed_dates
	# Extract each bulk part individually
	for file in $(find . -name "$client-$network-snapshot-bulk-*-part-*" -print | sort); do
		zstdcat "$file" | tar xvf - -C $extract_dir

		# Delete the file after processing it
		rm "$file"
	done

	# Extract each incremental part individually
	for file in $(find . -name "$client-$network-snapshot-*-part-*" -print | sort); do
      ...
      if [[ -z "${processed_dates[$date_stamp]}" ]]; then
		...
		zstdcat $client-$network-snapshot-${date_stamp}-part* | pv | tar -xf - -C .
      fi
  done
  ...
  for file in $(find . -name "$client-$network-snapshot-*-part-*" -print | sort); do
      ...
      if [[ -z "${processed_dates[$date_stamp]}" ]]; then
		...
		zstdcat $client-$network-snapshot-${date_stamp}-part* | pv | tar -xf - -C . --strip-components=3
      fi
  done