...
export async function matchPools() {
    const dataDir3 =
        "/mnt/d/code/arbitrage/polybot-live/polybotv3/data/validPairs/v3/";
    ...
    for (let i = 0; i < filesA.length; i++) {
        ...
        for (let j = 0; j < filesB.length; j++) {
            ...
            const fileJSBIame = fileB.split(".")[0];
            const dataB = require(path.join(dataDirAlg, fileB));

            if (dataA && dataB) {
                console.log("loop1", fileAName, fileJSBIame);
                ...
                outerLoop: for (const poolA of dataA) {
                    for (const poolB of dataB) {
                        ...
                        if (
                            ...
                            let PoolA = new Contract(
                                poolA.poolID,
                                IUniswapV3Pool,
                                provider,
                            );
                            let PoolB = new Contract(
                                poolB.poolID,
                                IAlgebraPool,
                                provider,
                            );
                            ...
                            let poolBInfo: PoolInfo = {
                                exchange: `${fileJSBIame}`,
                                protocol:
                                    fileJSBIame === "UNI" ? "UNIV3" : "ALG",
                                id: poolB.poolID,
                                fee: poolB.fee,
                ...
                const allMatches: V3Matches = {
                    exchangeA: `${fileAName} `,
                    exchangeB: `${fileJSBIame} `,
                    matches: matchingPools,
                };
                fs.writeFile(
                    path.join(matchesDir, `${fileAName}${fileJSBIame}.json`),
                    JSON.stringify(allMatches, null, 2),
                    function (err) {
                        if (err) return console.log(err);
                        console.log(
                            `Matching pairs written to ${path.join(
                                matchesDir,
                                `${fileAName}${fileJSBIame}.json`,
                            )} `,
                        );
                    },
                );
