...
import { fu } from "../scripts/modules/convertJSBI";
...
export async function getGas2WMATICArray(): Promise<ToWMATICPool[]> {
            ...
            for (let token in gasTokens) {
                if (gasTokens[token] !== wmaticID) {
                    ...
                    if (uniswapV3Exchange[exchange].protocol === "UNIV3") {
                        ...
                        const tokenOut =
                            token0.id == wmaticID ? token0 : token1;
    ...
    async function compareLiquidity(
        ToWMATICPools: ToWMATICPool[],
    ): Promise<ToWMATICPool[]> {
